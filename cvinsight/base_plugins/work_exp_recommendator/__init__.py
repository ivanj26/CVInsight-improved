...
class WorkExperienceRecommendator(RecommendatorPlugin):
    """ Recommendator plugin for work experience description. """
    ...
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="work_experience_recommendator",
            version="1.0.0",
            description="Generate recommendation about work experience description from context information",
...
Give me 8 examples to showcase my job description, make sure it has quantifiable metric. Do not forget replace the quantifiable metric with [Insert the number], so user can change its own metric.
You can also mix your answers by rephrasing my current job desc (e.g. 4 recommendation example + 4 rephrasing example), if description above exists. Please make it concise by at max 2 paragraphs for each recommendation and
...
"""
    ...
    def generate(self, data: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        result, token_usage = self.llm_service.generate_content(input_data, max_token=800)
